...
import { findClosestRelatedResource, findL1FromRef, memoizedGetter, resolvedGet } from '../../core/lib/helpers-internal';
...
export class BucketReflection {
  ...
  public get bucket(): CfnBucket {
    if (!this._bucket) {
      throw new UnscopedValidationError(`Unable to find underlying resource for ${this.ref.node.path}. Please pass the resource construct directly.`);
    }
    return this._bucket;
  }
   ...
   */
  get disallowPublicAccess(): boolean {
    return resolvedGet(this._bucket, 'publicAccessBlockConfiguration.blockPublicPolicy', false) === true;
  }
